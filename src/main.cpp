...
{
    if (tx.nVersion > CTransaction::TXDZEEL_VERSION) {
        reason = "version";
        return false;
    }
...
// ----------- TesseractX transaction scanning -----------
    ...
    bool InodePayments = false;

    if(nTime > START_INODE_PAYMENTS) InodePayments = true;
    ...
    if(InodePayments)
    {
        ...
        if(IsProofOfStake() && pindex != NULL){
            if(pindex->GetBlockHash() == hashPrevBlock){
                CAmount inodePaymentAmount = GetInodePayment(pindex->nHeight+1, vtx[1].GetValueOut());
                bool fIsInitialDownload = IsInitialBlockDownload();

                // If we don't already have its previous block, skip inode payment step
                if (!fIsInitialDownload && pindex != NULL)
                {
                    ...
                    CScript payee;
                    if(!inodePayments.GetBlockPayee(pindexBest->nHeight+1, payee) || payee == CScript()){
                        foundPayee = true; //doesn't require a specific payee
                        foundPaymentAmount = true;
                        foundPaymentAndPayee = true;
                        if(fDebug) { LogPrintf("CheckBlock() : Using non-specific inode payments %d\n", pindexBest->nHeight+1); }
                    }

                    for (unsigned int i = 0; i < vtx[1].vout.size(); i++) {
                        if(vtx[1].vout[i].nValue == inodePaymentAmount )
                            foundPaymentAmount = true;
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            foundPayee = true;
                        if(vtx[1].vout[i].nValue == inodePaymentAmount && vtx[1].vout[i].scriptPubKey == payee)
                            foundPaymentAndPayee = true;
                    }
                    ...
                    if(!foundPaymentAndPayee) {
                        ...
                        if(fDebug) { LogPrintf("CheckBlock() : Couldn't find inode payment(%d|%d) or payee(%d|%s) nHeight %d. \n", foundPaymentAmount, inodePaymentAmount, foundPayee, address2.ToString().c_str(), pindexBest->nHeight+1); }
                        return DoS(100, error("CheckBlock() : Couldn't find inode payment or payee"));
                    } else {
                        if(fDebug) { LogPrintf("CheckBlock() : Found inode payment %d\n", pindexBest->nHeight+1); }
                    }
                } else {
                    if(fDebug) { LogPrintf("CheckBlock() : Is initial download, skipping inode payment check %d\n", pindexBest->nHeight+1); }
                }
            } else {
                if(fDebug) { LogPrintf("CheckBlock() : Skipping inode payment check - nHeight %d Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
            }
        } else {
            if(fDebug) { LogPrintf("CheckBlock() : pindex is null, skipping inode payment check\n"); }
        }
    } else {
        if(fDebug) { LogPrintf("CheckBlock() : skipping inode payment checks\n"); }
    }
...
#ifdef ENABLE_WALLET
// navcoin: attempt to generate suitable proof-of-stake
...
bool static AlreadyHave(CTxDB& txdb, const CInv& inv)
{
    ...
    case MSG_HUB:
        return mapHubs.count(inv.hash);
    case MSG_INODE_WINNER:
        return mapSeenInodeVotes.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_HUB) {
                    if(mapHubs.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapHubs[inv.hash];
                        pfrom->PushMessage("hub", ss);
                        pushed = true;
                    }
                }
                if (!pushed && inv.type == MSG_INODE_WINNER) {
                    if(mapSeenInodeVotes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        int a = 0;
                        ss.reserve(1000);
                        ss << mapSeenInodeVotes[inv.hash] << a;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else
    {
        ...
        ProcessMessageInode(pfrom, strCommand, vRecv);
        ProcessMessageTesseractX(pfrom, strCommand, vRecv);
        ProcessHub(pfrom, strCommand, vRecv);
