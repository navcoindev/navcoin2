...
//Dark  features
bool fINode = false;
string strINodePrivKey = "";
string strINodeAddr = "";
bool fLiteMode = false;
int nTesseractXDepth = 1;
int nAnonsendRounds = 2;
int nAnonymizeNavCoinAmount = 500;
int nLiquidityProvider = 0;
/** Hub enforcement enabled time */
int64_t enforceInodePaymentsTime = 4085657524;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NavCoin2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NavCoin2
    // Mac: ~/Library/Application Support/NavCoin2
    // Unix: ~/.navcoin2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NavCoin2";
#else
    fs::path pathRet;
...
boost::filesystem::path GetInodeConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-mnconf", "inode.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;
    return pathConfigFile;
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "navcoind.pid"));
