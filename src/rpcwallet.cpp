...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current NavCoin address for receiving payments to this account.");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <navcoinaddress> <amount> [comment] [comment-to] [narration] [anon-destination]\n"
    ...
    if (params.size() > 3 && params[3].type() != null_type && !params[3].get_str().empty())
 	...
 	{
    unsigned int DZeelMaxLen = MAX_DZEEL;
        if (nBestHeight >= (int)DZEEL_HEIGHT)
            DZeelMaxLen = MAX_DZEEL;
         dzeel = params[5].get_str();
    ...
    string strError = pwalletMain->SendMoneyToDestination(address.Get(), nAmount, sNarr, wtx, false, dzeel);
    if (strError != "")
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <navcoinaddress> [minconf=1]\n"
            "Returns the total amount received by <navcoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <tonavcoinaddress> <amount> [minconf=1] [comment] [comment-to] [narration] [anon-destination]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
    ...
    if (params.size() > 5 && params[5].type() != null_type && !params[5].get_str().empty())
	 ...
	 unsigned int DZeelMaxLen = MAX_DZEEL;
        if (nBestHeight >= (int)DZEEL_HEIGHT)
            DZeelMaxLen = MAX_DZEEL;
          dzeel = params[7].get_str();
		if (dzeel.length() > DZeelMaxLen)
			dzeel.resize(DZeelMaxLen);
 	}
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendmany <fromaccount> {address:amount,...} [minconf=1] [comment] [anon-destination]\n"
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, strDZeel, nChangePos, strFailReason);
    if (!fCreated)
    {
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; NavCoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
// NavCoin: resend unconfirmed wallet transactions
...
Value getnewsecretaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getnewsecretaddress [label]\n"
            "Returns a new NavCoin secret address for receiving payments anonymously.  ");
    ...
    std::string sError;
    if (!pwalletMain->NewSecretAddress(sError, sLabel, sxAddr))
        throw runtime_error(std::string("Could get new secret address: ") + sError);
    
    if (!pwalletMain->AddSecretAddress(sxAddr))
        throw runtime_error("Could not save to wallet.");
    
    return sxAddr.Encoded();
}

Value listsecretaddresses(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "listsecretaddresses [show_secrets=0]\n"
            "List owned secret addresses.");
    ...
    //for (it = pwalletMain->secretAddresses.begin(); it != pwalletMain->secretAddresses.end(); ++it)
    BOOST_FOREACH(CSecretAddress sit, pwalletMain->secretAddresses)
    {
	CSecretAddress* it = &(sit);
        ...
        if (fShowSecrets)
        {
            ...
            result.push_back(Pair("Secret Address", objA));
        } else
        {
            result.push_back(Pair("Secret Address", it->Encoded() + " - " + it->label));
        };
    };
...
Value importsecretaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2)
        throw runtime_error(
            "importsecretaddress <scan_secret> <spend_secret> [label]\n"
            "Import an owned secret addresses.");
    ...
    //std::set<CSecretAddress>::iterator it;
    //for (it = pwalletMain->secretAddresses.begin(); it != pwalletMain->secretAddresses.end(); ++it)
    BOOST_FOREACH(CSecretAddress it, pwalletMain->secretAddresses)
    {
        CSecretAddress &sxAddrIt = const_cast<CSecretAddress&>(it); //*it);
            ...
            && sxAddrIt.spend_pubkey == sxAddr.spend_pubkey)
        {
            if (sxAddrIt.scan_secret.size() < 1)
            {
                ...
                fFound = true; // update secret address with secrets
                break;
            };
            
            result.push_back(Pair("result", "Import failed - secret address exists."));
            return result;
        };
    };
    ...
    {
        pwalletMain->secretAddresses.insert(sxAddr);
...
Value sendtosecretaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtosecretaddress <secret_address> <amount> [comment] [comment-to] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
    ...
    {
        result.push_back(Pair("result", "Invalid NavCoin secret address."));
        return result;
    };
    ...
    std::string sError;
    if (!pwalletMain->SendSecretMoneyToDestination(sxAddr, nAmount, sNarr, wtx, sError))
...
Value scanforsecrettxns(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "scanforsecrettxns [fromHeight]\n"
            "Scan blockchain for owned secret transactions.");
    ...
    pwalletMain->nSecret = 0;
    pwalletMain->nFoundSecret = 0;
    ...
    printf("Found %u secret transactions in blockchain.\n", pwalletMain->nSecret);
    printf("Found %u new owned secret transactions.\n", pwalletMain->nFoundSecret);
    
    char cbuf[256];
    snprintf(cbuf, sizeof(cbuf), "%u new secret transactions.", pwalletMain->nFoundSecret);
