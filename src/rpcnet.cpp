...
Value addanonserver(const Array& params, bool fHelp)
{
    string strCommand;
    if (params.size() == 2)
        strCommand = params[1].get_str();
...
Value getaddedanonserverinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getaddedanonserverinfo <dns> [node]\n"
            "Returns information about the given added anon node, or all added nodes\n"
    ...
    {
        LOCK(cs_vAddedAnonServers);
        BOOST_FOREACH(string& strAddNode, vAddedAnonServers)
