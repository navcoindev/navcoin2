...
{
    // NavCoin: check prefix
    if(uri.scheme() != QString("navcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("navcoin://"))
    {
        uri.replace(0, 12, "navcoin:");
    }
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the INavCoinLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query INavCoinLink for the IPersistFile interface for
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NavCoin-Qt") + " " + tr("version") + " " +
