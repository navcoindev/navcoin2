...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNavCoinAmount"))
        settings.setValue("nAnonymizeNavCoinAmount", 1000);

    nAnonsendRounds = settings.value("nAnonsendRounds").toLongLong();
    nAnonymizeNavCoinAmount = settings.value("nAnonymizeNavCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeNavCoinAmount"))
        SoftSetArg("-anonymizenavcoinamount", settings.value("nAnonymizeNavCoinAmount").toString().toStdString());
#ifdef USE_NATIVE_I2P
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case AnonsendRounds:
            ...
            break;
        case anonymizeNavCoinAmount:
            nAnonymizeNavCoinAmount = value.toInt();
            settings.setValue("nAnonymizeNavCoinAmount", nAnonymizeNavCoinAmount);
            emit anonymizeNavCoinAmountChanged(nAnonymizeNavCoinAmount);
break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
