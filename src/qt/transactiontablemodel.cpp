...
{
    columns << QString() << tr("Date") << tr("Type") << tr("Address") << tr("Amount") << tr("Anon-Destination-Address");
...
QString TransactionTableModel::formatDZeel(const TransactionRecord *wtx, bool tooltip) const
 {
     switch(wtx->type)
     {
     case TransactionRecord::RecvFromOther:
         return QString::fromStdString(wtx->dzeel);
     case TransactionRecord::RecvWithAddress:
         return QString::fromStdString(wtx->dzeel);
     case TransactionRecord::SendToAddress:
     case TransactionRecord::SendToOther:
...
QString TransactionTableModel::formatTooltip(const TransactionRecord *rec) const
{
       ...
       rec->type==TransactionRecord::SendToAddress || rec->type==TransactionRecord::RecvWithAddress)
    {
		...
		if (rec->dzeel.length() > 0)
         {
             tooltip += QString("\n") + formatDZeel(rec, true);
         }
    }
    return tooltip;
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        } else if (role == Qt::ToolTipRole)
        {
			...
			case DZeel:
				return tr("Anon Destination Address");
            }
        }
    }
    return QVariant();
