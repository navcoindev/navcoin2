...
struct AddressTableEntry
{
    ...
    AddressTableEntry() {}
    AddressTableEntry(Type type, const QString &label, const QString &address, const bool &secret = false):
        type(type), label(label), address(address), secret(secret) {}
};
...
public:
    ...
    void refreshAddressTable()
    {
        cachedAddressTable.clear();
        {
            ...
            for (it = wallet->secretAddresses.begin(); it != wallet->secretAddresses.end(); ++it)
            {
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            if (IsSecretAddress(strTemp))
            {
                strValue = value.toString().toStdString();
                wallet->UpdateSecretAddress(strTemp, strValue, false);
            } else
            {
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if(type == Send)
    {
        if (strAddress.length() > 75)
        {
            ...
            // -- Check for duplicate addresses
            {
                ...
                if (wallet->secretAddresses.count(sxAddr))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                };
                ...
                wallet->AddSecretAddress(sxAddr);
            }
    ...
    else if(type == Receive)
    {
        ...
        if (addressType == AT_Secret)
        {
            CSecretAddress newSecretAddr;
            std::string sError;
            if (!wallet->NewSecretAddress(sError, strLabel, newSecretAddr)
                || !wallet->AddSecretAddress(newSecretAddr))
            {
                editStatus = KEY_GENERATION_FAILURE;
                return QString();
            }
            strAddress = newSecretAddr.Encoded();
        } else
        {
            CPubKey newKey;
...
{
    {
        ...
        if (sAddr.length() > 75)
        {
            ...
            std::set<CSecretAddress>::iterator it;
            it = wallet->secretAddresses.find(sxAddr);
            if (it == wallet->secretAddresses.end())
                return QString();
